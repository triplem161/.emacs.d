...
#+begin_src emacs-lisp
  ...
  ;;   :defer t
  ;;   :config
  ;;   (add-to-list 'eglot-server-programs '(python-mode . ("c:/Users/Marion/AppData/Roaming/.emacs.d/.cache/lsp/npm/pyright/pyright" "-w --outputjson"))))
#+end_src
** Emmet
   #+BEGIN_SRC emacs-lisp
...
#+begin_src emacs-lisp
  (use-package ledger-mode
    ...
    (setq ledger-reports '(("assets" "%(binary) -f %(ledger-file) bal ^assets")
                           ("bal" "%(binary) -f %(ledger-file) bal")
                           ("reg" "%(binary) -f %(ledger-file) reg")
                           ("budget" "%(binary) -f %(ledger-file) --p %(month) budget -R")
                           ("unbudgeted" "%(binary) -f %(ledger-file) --p %(month) bal --unbudgeted ^expenses")
                           ("groceries avg month" "%(binary) -f %(ledger-file) -p \"this year\" --monthly --average register ^expenses:groceries"))))
  ;; '(ledger-reports
